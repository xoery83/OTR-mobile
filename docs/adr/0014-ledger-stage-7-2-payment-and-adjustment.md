...
Date: 2026-09-12
Status: Accepted; Stage 7.2A implemented and accepted at its engineering gate
...
migrations (schema/RPC, narrow internal helper grants, and a one-successor
Payment-lineage guard). It reuses the existing Settlement aggregate, durable sync queue,
Backend idempotency ledger, incremental pull, audit table, and currency helpers.
No payment provider, new state framework, export, Adjustment implementation,
Stage 7.3 work, Production change, or legacy Web change is authorized.
