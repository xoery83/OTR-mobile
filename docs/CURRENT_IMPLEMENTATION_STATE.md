...
- A real test-only Hosted Dev Member `review2-device-member-20260917@otr.invalid` is linked to the synthetic baseline Journey. Its 16-character random password is stored only in the local macOS login Keychain service `com.xoery.otrmobile.review2.hosted-dev.20260917` (never in source/logs); Keychain readback and real password Auth sign-in passed. Real Auth → Backend checks passed Owner-no-split, Creator-only, Payer-only, split-eligibility loss, cross-date and cross-currency negatives, and cosmetic edit stability. The repeatable check is `scripts/supabase/validate-review-v2-remaining-hosted.mjs`; eight incomplete first-run Expenses and three completed negative-rule Expenses were recoverably soft-deleted via Backend. Four eligibility fixture pairs are retained for device checks.
- The iPhone 17 Pro Simulator completed genuine Owner → test Member → Owner switching through Account management, with both accounts remembered. Their Review decisions stayed separate; Member's local active Split-only Finding disappeared after its split became zero while Owner still saw it. The device showed sync up to date after Member login and after Owner switch. **Not a full acceptance pass:** device network-disconnected ACK → reconnect remains unrun because Simulator Settings and Control Center expose no working Simulator-only Airplane/Wi-Fi control. Physical iPhone installation succeeded, but Device Hub cannot show its iOS 26.6 screen (requires iOS 27); iPhone Mirroring requires personal Mac login. Phase 3 UI work remains gated. Next checkpoint: real-device offline convergence, then decide acceptance.
