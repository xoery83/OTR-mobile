...
- `docs/ledger/CURRENCY_FX_PHASE_A_DATE_FINDING.md` proves existing `occurred_at` cannot unambiguously recover every historical Journey-local calendar date: new form emits a local `YYYY-MM-DD`, Dev casts it to `timestamptz`, and imported/other values may be UTC/offset timestamps without original timezone intent. **Stop before Phase B**; approve explicit `economic_date` contract, forward migration and ambiguous-history handling before connecting a historical FX provider. Do not infer from UTC or device timezone.
- Governing documents: `docs/ledger/CURRENCY_FX_DOMAIN_AUDIT.md` and `docs/ledger/CURRENCY_FX_CONVERGENCE_PLAN.md`. Future 7-day lookback and finalized Journey Currency lock are product-approved, but Phase B–F implementation is not authorized. No Production access.
